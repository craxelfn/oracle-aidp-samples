...
Supports a ``--mock-secret`` mode (env var ``AIDP_FUSION_AUTOPILOT_MOCK_SECRETS``)
...
def fetch_secret(secret_ocid: str, *, oci_profile: str = "DEFAULT") -> str:
    ...
    Test-mode override: set ``AIDP_FUSION_AUTOPILOT_MOCK_SECRETS`` to a
    ``ocid1=value;ocid2=value`` string and this returns the matching value
    without touching OCI.
    """
    mock_env = os.environ.get("AIDP_FUSION_AUTOPILOT_MOCK_SECRETS")
    if mock_env:
        for entry in mock_env.split(";"):
            ...
            if key.strip() == secret_ocid:
                return val
        raise KeyError(
            f"AIDP_FUSION_AUTOPILOT_MOCK_SECRETS has no entry for {secret_ocid}"
        )
