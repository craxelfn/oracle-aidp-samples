"""Implementation of ``aidp-fusion-autopilot dashboard install --target oac``.
...
2. **Workbook content** — Snapshot register + restore (the only public path)
   ...
   ``aidp-fusion-autopilot bundle build-bar`` from a clean dev OAC containing
    ...
    if existing and not params.overwrite_connection:
        ...
        try:
            created = client.create_connection(
                ...
                description=(
                    "AIDP Fusion Autopilot JDBC connection — auto-installed by "
