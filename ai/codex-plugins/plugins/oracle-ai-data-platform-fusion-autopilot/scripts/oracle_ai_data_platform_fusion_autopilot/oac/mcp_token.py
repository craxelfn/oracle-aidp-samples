...
to **interactive browser auth**, which cannot complete inside the Codex MCP
...
route that works in Codex: credentials are supplied up front (the connector
...
#: in Codex's .mcp.json; no username, URL, or creds are committed).
    ...
    *,
    ...
    Args:
        connector_arg: the connector path. Defaults to
            ...
            which Codex expands at launch and which has no username embedded.
