...
description: >-
  Guide the governed manual OAC AIDP connection and dataset checkpoint, then verify the result through OAC MCP. Use after dataset advice, when creating or reusing an OAC dataset, or before workbook authoring.
---
...
python3 skills/aidp-fusion-autopilot/write_resume_checkpoint.py \
  ...
  --next-step "Reconnect Codex, verify oac-mcp-server, then resume $oac-dataset-setup" \
  --pending "Verify OAC dataset with search_catalog and describe_data"
```

Then restart or reconnect Codex and paste:
...
next: run the project-scoped dashboard mcp-setup command, reconnect Codex, then resume
```
