...
name: aidp-rest
description: >-
  Use AIDP control-plane REST primitives for OCI signing, discovery, notebook upload, job dispatch, polling, output retrieval, and marker parsing. Use when another workflow must operate a live AIDP cluster through REST.
---
...
- The `.aidp/cli.js` CLI flows defined in the workspace `CODEX.md` — those handle agent flows, not workspace data jobs.
