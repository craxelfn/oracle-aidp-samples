...
description: >-
  Conduct the complete Oracle Fusion to AIDP to OAC dashboard journey by composing the plugin's focused Skills and guarded CLI. Use when the user wants an analytics goal driven end to end or wants to resume a partially completed dashboard workflow.
---
...
| **1b** | **OAC MCP connect** (front-loaded prerequisite — see §below) | `oac-mcp-server` tools answer a live `search_catalog` ping | project-scoped `dashboard mcp-setup`, then **restart/reconnect Codex** | **always when dead** — staging the connector needs a Codex restart before its tools work; write the resume checkpoint, then PAUSE for the restart |
...
a **Codex restart/reconnect** before the tools come
...
1. **Probe liveness** — a cheap `oracle_analytics-search_catalog` ping (or
   `codex mcp list` → expect `oac-mcp-server ✔ Connected`).
...
3. **Dead → set up + restart.** From the customer project directory, run:
   ...
   **PAUSE with an explicit handoff**: *"OAC MCP staged — restart/reconnect
   Codex (`/mcp` → reconnect `oac-mcp-server`, or relaunch), then paste:
...
Before any pause that may cross a Codex restart/reconnect or a long
...
python3 skills/aidp-fusion-autopilot/write_resume_checkpoint.py \
  ...
  --next-step "Reconnect Codex, verify oac-mcp-server, then re-run $aidp-fusion-autopilot" \
...
downloaded the Codex plugin but not manually run `pip install -e`. Do not
...
- **OAC MCP liveness (Step 1b gate):** a cheap `oracle_analytics-search_catalog`
  ping, or `codex mcp list` (expect `oac-mcp-server ✔ Connected`). A failure /
...
- **OAC MCP not connected (Step 1b)** — stage it with `dashboard mcp-setup`,
  ...
  restart/reconnect Codex**; the tools can't activate inside the current
