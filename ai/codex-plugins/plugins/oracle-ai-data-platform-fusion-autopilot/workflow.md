...
| 1b. OAC MCP connect | Make OAC tools available before OAC phases | `dashboard mcp-setup` | Codex restart/reconnect |
...
After `dashboard mcp-setup`, Codex must restart or reconnect the MCP
...
Phase 8 is different from Phase 1b. Phase 1b equips the operator's Codex
session so autopilot can inspect and write OAC content. Phase 8 is the optional
handoff for end users who want their own Codex Desktop, Codex, Cline, or
...
Then start Codex from `demo-fusion-cfo/` and invoke:
...
For Codex, use the supported setup command:
...
Then restart or reconnect Codex and verify that `oac-mcp-server` is
...
For Codex, the same non-interactive auth guidance applies:
...
# Set up operator OAC MCP for Codex, then restart/reconnect
aidp-fusion-autopilot dashboard mcp-setup \
  --connector-js <path-to-oac-mcp-connect.js>
