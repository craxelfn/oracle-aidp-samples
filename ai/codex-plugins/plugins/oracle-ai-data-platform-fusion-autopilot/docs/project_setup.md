...
| Codex | Needed for the conversational skill/autopilot experience. |
| OAC MCP connector zip | Downloaded from OAC Profile -> MCP Connect. |

## Route 1: Codex Plugin

Recommended for users. Download the plugin, open Codex from a clean
customer bundle directory, and let `aidp-fusion-autopilot` drive setup and
delivery.

Run these slash commands in Codex:
...
Start Codex from `demo-fusion-cfo/`, not from the plugin repo, and ask
...
Codex MCP restart, destructive seed confirmation, and the manual OAC
...
Then restart or reconnect Codex and verify `oac-mcp-server` is connected.
...
For a live demo, open Codex from `demo-fusion-cfo/`, not from the plugin
...
- Codex has been restarted or reconnected and `oac-mcp-server` is live.
